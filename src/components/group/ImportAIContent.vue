...
// Filtered content based on format and search
const filteredContent = computed(() => {
  let filtered = userAIContent.value

  // Filter by content type
  if (selectedFormat.value !== 'all') {
    filtered = filtered.filter((content) => {
      const contentType = content.outputFormat?.toLowerCase()
      return contentType === selectedFormat.value
    })
  }
  ...
  if (searchQuery.value.trim()) {
    ...
    filtered = filtered.filter((content) => {
      ...
      const preview = getContentPreview(content).toLowerCase()
...
const formatOptions = [
  { value: 'all', label: 'All Content', icon: 'Cards' },
...
const selectAll = () => {
  selectedContent.value = filteredContent.value.map((content) => String(content.id))
}
    ...
    <div class="bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
      ...
      <div class="flex items-center justify-between p-6 border-b border-gray-200">
        <div class="flex items-center gap-3">
          <div class="relative">
            <svg class="w-6 h-6 drop-shadow-sm text-blue-600" fill="currentColor" viewBox="0 0 24 24">
              <g fill="none">
          ...
          <div>
            <h3 class="text-xl font-bold text-gray-900">Import AI Content</h3>
            <p class="text-gray-500">Select content from your AI-generated library</p>
          </div>
        </div>
        <button
          @click="closeModal"
      ...
      <div class="p-6">
        <!-- Filters -->
        <div class="bg-white/60 backdrop-blur-sm rounded-xl border border-white/50 p-4 mb-6">
        ...
        <div v-else-if="filteredContent.length === 0" class="text-center py-12">
          ...
          <h3 class="text-lg font-medium text-gray-900 mb-2">
            {{ userAIContent.length === 0 ? 'No AI content found' : 'No content matches your filters' }}
          </h3>
          <p class="text-gray-500 mb-4">
            {{ userAIContent.length === 0
        ...
        <div v-else class="space-y-3">
          <div
            ...
            :key="content.id"
            class="flex items-start gap-4 p-4 border border-gray-200 rounded-lg hover:border-blue-300 transition-colors cursor-pointer"
            :class="
      ...
      <div class="flex gap-3 p-6 border-t border-gray-200">
        <button
          @click="closeModal"
          class="flex-1 px-4 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg font-medium transition-colors duration-200"
        >
          Cancel
        </button>
        <button
          ...
          class="flex-1 px-4 py-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-lg font-medium transition-all duration-200 flex items-center justify-center gap-2"
        >
