<template>
  ...
  >
    ...
    >
      ...
      <!-- Progress Bar -->
      <div
        v-if="['info', 'success'].includes(notification.type)"
        class="absolute bottom-0 left-0 h-1 bg-current opacity-20 transition-all duration-500"
...
const getNotificationClasses = (type: Notification['type']) => {
  const baseClasses = 'flex items-start p-4 rounded-lg shadow-lg cursor-pointer transform transition-all duration-300 hover:scale-105'
...
onMounted(() => {
  progressInterval = setInterval(() => {
    progressWidth.value = Math.max(0, progressWidth.value - 2)
  }, 100)
})
